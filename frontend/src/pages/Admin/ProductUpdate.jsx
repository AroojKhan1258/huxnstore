...
const AdminProductUpdate = () => {
  ...
  const [metaKeywords, setMetaKeywords] = useState(
    Array.isArray(productData?.metaKeywords) ? productData.metaKeywords.join(", ") : ""
  );
  ...
  useEffect(() => {
    if (productData && productData._id) {
      ...
      setMetaKeywords(
        Array.isArray(productData.metaKeywords)
  ...
  const handleAiSEO = async () => {
    ...
    try {
      ...
      setMetaKeywords(Array.isArray(data.metaKeywords) ? data.metaKeywords.join(", ") : data.metaKeywords || "");
      toast.success("✨ SEO fields auto-generated!");
    } catch { toast.error("AI SEO generation failed"); }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Send as JSON — image is already uploaded as a URL
            ...
            <div className="p-3">
              ...
              <div className="mt-5 border-t border-gray-600 pt-4">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-white font-semibold">SEO &amp; Meta Details</h2>
                  <button type="button" onClick={handleAiSEO} disabled={aiLoading} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-violet-600 to-pink-600 hover:from-violet-500 hover:to-pink-500 text-white text-xs font-bold rounded-lg disabled:opacity-50 transition-all">
